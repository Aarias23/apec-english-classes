const questions = [
  ...
  { section: "Everyday English", type: "True / False", text: "The sentence ‘She doesn’t study on Sunday’ is grammatically correct.", instruction: "Write True or False in Blackboard.", answer: "True" },
  ...
  { section: "Town to Table", type: "True / False", text: "You can borrow a book at the library.", instruction: "Write True or False in Blackboard.", answer: "True" },
...
function resetTimer() { stopTimer(); timeLeft = Number($("timerLength").value); $("timerDisplay").textContent = formatTime(timeLeft); $("timerDisplay").style.color = ""; }
function startTimer() {
  if (timerId) return;
  timerId = setInterval(() => {
...
$("questionMap").innerHTML = questions.map((question, index) => `<button type="button" data-index="${index}" class="${index === 11 || index === 23 ? "section-start" : ""}" aria-label="Go to question ${index + 1}">${index + 1}</button>`).join("");
...
$("toggleOptions").addEventListener("click", () => { questionContent.hidden = !questionContent.hidden; $("toggleOptions").textContent = questionContent.hidden ? "Show options" : "Hide options"; $("toggleOptions").setAttribute("aria-expanded", String(!questionContent.hidden)); });
$("fullscreenButton").addEventListener("click", async () => { const target = workspace.hidden ? document.documentElement : $("examSlide"); if (!document.fullscreenElement) await target.requestFullscreen(); else await document.exitFullscreen(); });
